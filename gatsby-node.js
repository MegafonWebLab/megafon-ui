...
const generateIndex = files => {
    ...
    const imports = sorted.map(
        ({ name, path: cPath, ext: extension }) =>
            `export { default as ${name} } from '${cPath.replace(
                extension,
                ""
            )}';`
    );
...
exports.onPreInit = () => {
    fs.writeFile("../intro.mdx", createReadmeMdx(), err => {
        if (!err) {
            console.log("intro.mdx created");
        }
    });

    const components = glob.sync(
        "../packages/ui-core/src/components/**/*.{tsx,ts}",
        { ignore: [testsReg, doczReg] }
    );

    fs.writeFile(indexTs, generateIndex(components), err => {
        if (!err) {
            console.log("ui-core/src/index.ts created");
        }
    });
};
...
exports.onCreateWebpackConfig = ({ actions, getConfig, plugins, loaders }) => {
    ...
    actions.setWebpackConfig({
        ...
        resolve: {
            ...
            alias: {
                "@megafon/ui-core": resolve(
                    __dirname,
                    "../packages/ui-core/src"
                ),
                "@megafon/ui-helpers": resolve(
                    __dirname,
