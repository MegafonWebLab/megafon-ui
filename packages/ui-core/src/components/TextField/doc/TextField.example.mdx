...
import { wrapperDefaultWidthStyle, wrapperWideWidthStyle } from './TextField.docz';
...
## Валидация
Клик по иконке очищает поле по умолчанию. Если установлены иконка и `onCustomIconClick`, то очищение перестает работать.
