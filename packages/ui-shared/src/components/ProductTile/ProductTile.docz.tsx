import * as React from 'react';
import MegafonTv from 'icons/Service-logos/24/MF-TV.svg';

export const ProductTileWrapper = ({children}) => {
    return <div style={{ padding: '20px', display: 'flex', flexWrap: 'wrap' }}>{children}</div>;
};

export const tariff = {
    topBadgeTitle: 'Тариф–конструктор от 250 ₽',
    link: '/tariffs/vklyuchaysya/smotri.html',
    topBadgeLink: '#',
    morelinkText: 'Подробнее о тарифе',
    startCallsIndex: 3,
    startTrafficIndex: 1,
    cookieCallsIndex: 0,
    cookieTrafficIndex: 3,
    showMorelink: true,
    topBadgeLinkTarget: '_self',
    buyLinkTarget: '_self',
    connectLinkTarget: '_self',
    linkTarget: '_self',
    buyButtonText: 'Выбрать',
    showBuyButton: true,
    connectLink: '#',
    connectButtonText: 'Подключить',
    showConnectButton: true,
    shopTag: 'tag',
    'title': 'Включайся! Смотри',
    'secondParamsHead': 'Дополнительные возможности',
    'payment': {
        title: 'При покупке новой SIM–карты',
        'value': '1000',
        'unitExtra': 'за 30 дней',
        'unitValue': '₽',
        'unit': '₽ за 30 дней',
        'discount': '600',
    },
    'packs': [
        {
            'value': 1500,
            'unit': 'минут',
        },
        {
            'value': 100,
            'unit': 'смс',
            isDelim: true,
            title: 'Безлимит',
        },
        {
            'value': 3,
            'unit': 'ГБ',
        },
    ],
    'firstParams': {
        'title': 'Параметры',
        'caption': '',
        'items': [
            {
                'title': 'VIP-тариф',
                'caption': 'Привилегии обслуживания и другое',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'МегаФон ТВ',
                'caption': 'Безлимитный трафик и&nbspпакеты кино, сериалов и&nbspТВ-каналов на&nbspлюбой вкус',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    'secondParams': [
        {
            'title': 'Кэшбэк 20%',
            'footnote': 'Участвуйте в&nbspпрограмме и&nbspтратьте кэшбэк на&nbspсервисы и&nbspпокупки в&nbspсалонах и&nbspинтернет-магазине МегаФона',
            'value': '0',
            'unit': '',
            'svgIcon': <MegafonTv />,
        },
        {
            'title': 'Безлимитные SMS',
            'footnote': 'Участвуйте в&nbspпрограмме и&nbspтратьте кэшбэк на&nbspSMS',
            'value': '55',
            'unit': '₽ за 30 дней',
            'svgIcon': <MegafonTv />,
        },
    ],
    'cashback': {
        'title': 'Кэшбэк',
        'value': 20,
        'unit': '%',
    },
    'info': {
        'billingIds': [3, 5],
        'sms': '05007897',
        'smsText': 'ДА',
        'smsSite': null,
        'ivr': '05007897',
        'ussd': '<span class="c-number-call"><b class="asterisk">*</b><b class="num">789</b><span class="c-number-call"><b class="asterisk">*</b><b class="num">7</b><b class="dash">#</b><b class="pipe"></b></span>',
        'lkLink': 'https://lk.megafon.ru/tariffs/',
        'title': 'Включайся! Смотри',
        'id': 1598,
    },
};

export const servicePacks = [
    {
        'buyLink': '&options=null',
        'shopTag': 'tag1',
        'calls': {
            'value': 300,
            'unit': 'минут',
        },
        'traffic': {
            'value': 6,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '400',
            'discount': '200',
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'shopTag': 'tag2',
        'calls': {
            'value': 650,
            'unit': 'минут',
        },
        'traffic': {
            'value': 6,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '590',
            'discount': '300',
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'shopTag': 'tag3',
        'calls': {
            'value': 300,
            'unit': 'минут',
        },
        'traffic': {
            'value': 18,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '590',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'shopTag': 'tag4',
        'calls': {
            'value': 650,
            'unit': 'минут',
        },
        'traffic': {
            'value': 18,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '650',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'shopTag': 'tag5',
        'calls': {
            'value': 900,
            'unit': 'минут',
        },
        'traffic': {
            'value': 28,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '840',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'shopTag': 'tag6',
        'calls': {
            'value': 300,
            'unit': 'минут',
        },
        'traffic': {
            'value': 28,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '640',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'shopTag': 'tag7',
        'calls': {
            'value': 300,
            'unit': 'минут',
        },
        'traffic': {
            'value': 38,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '690',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 650,
            'unit': 'минут',
        },
        'traffic': {
            'value': 28,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '700',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 650,
            'unit': 'минут',
        },
        'traffic': {
            'value': 38,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '700',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 750,
            'unit': 'минут',
        },
        'traffic': {
            'value': 6,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '670',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 750,
            'unit': 'минут',
        },
        'traffic': {
            'value': 18,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '720',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 750,
            'unit': 'минут',
        },
        'traffic': {
            'value': 28,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '770',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 750,
            'unit': 'минут',
        },
        'traffic': {
            'value': 38,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '820',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 900,
            'unit': 'минут',
        },
        'traffic': {
            'value': 6,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '770',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 900,
            'unit': 'минут',
        },
        'traffic': {
            'value': 18,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '790',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
    {
        'buyLink': '&options=null',
        'calls': {
            'value': 900,
            'unit': 'минут',
        },
        'traffic': {
            'value': 38,
            'unit': 'ГБ',
        },
        'payment': {
            'value': '900',
            'discount': null,
        },
        'options': [
            {
                'title': 'Безлимитные SMS',
                'value': '55',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Безлимитный интернет',
                'value': '0',
                'unit': '₽ за 30 дней',
                'footnote': '',
                'svgIcon': <MegafonTv />,
            },
            {
                'title': 'Кэшбэк 20%',
                'value': '0',
                'unit': '',
                'footnote': 'Участвуйте в программе и тратьте кэшбэк на сервисы и покупки в салонах и интернет-магазине МегаФона',
                'svgIcon': <MegafonTv />,
            },
        ],
    },
];
